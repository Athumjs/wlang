...
void addToken(struct Tokens *tokens, struct Arena *arena, enum TokenType t,
              union Literal v, int l, int c) {
  if (tokens->length == tokens->capacity) {
    size_t oldCap = tokens->capacity;
    tokens->capacity *= 2;
    struct Token *temp =
        arena_alloc(arena, tokens->capacity * sizeof(struct Token));
...
void addTokenString(struct Tokens *tokens, struct Arena *arena,
                    enum TokenType t, const char *v, size_t s, int l, int c) {
  union Literal literal = {.string.start = v, .string.length = s};
  addToken(tokens, arena, t, literal, l, c);
}

void addTokenNum(struct Tokens *tokens, struct Arena *arena, enum TokenType t,
                 const char *v, size_t s, int b, int l, int c) {
...
enum Mode { Mode_Normal, Mode_Text, Mode_Code } mode = Mode_Normal;
...
void lexer(const char *filename, const char *code, struct Tokens *tokens,
  ...
  while (index < len) {
    if (mode == Mode_Text) {
      ...
      addTokenString(tokens, arena, LITERAL_STRING, code + start, index - start,
                     line, col - (index - start));
      if (code[index] == '"') {
        mode = Mode_Normal;
        addTokenString(tokens, arena, TEMPLATE_END, "", 0, line,
                       col - (index - start));
      }
    ...
    if (code[index] == '/' && code[index + 1] == '*') {
      NEXT();
      NEXT();
      while (code[index] && (code[index] != '*' || code[index + 1] != '/'))
        NEXT();
    ...
    if (code[index] == '_' || (code[index] >= 'a' && code[index] <= 'z') ||
        (code[index] >= 'A' && code[index] <= 'Z')) {
      int start = index;
      while (code[index] == '_' || (code[index] >= 'a' && code[index] <= 'z') ||
             (code[index] >= 'A' && code[index] <= 'Z') ||
             isNum(code[index], 10))
        NEXT();

#define X(name, str)                                                           \
  if (index - start == strlen(str) &&                                          \
      memcmp(code + start, str, index - start) == 0) {                         \
    addTokenString(tokens, arena, name, code + start, index - start, line,     \
                   col - (index - start));                                     \
    continue;                                                                  \
  }
      KEYWORDS
#undef X
    ...
    if (isNum(code[index], 10)) {
      ...
      while (code[index]) {
        if (i == 29)
          errorLang(filename, line, col, "the number constant too large");
        if (code[index] == '_')
          NEXT();
      ...
      buffer[i] = '\0';
      addTokenNum(tokens, arena, type, buffer, i, base, line,
                  col - (index - start));
      continue;
    }
...
#define X(name, str)                                                           \
  if (sizeof(str) - 1 > bestLen &&                                             \
      memcmp(code + index, str, sizeof(str) - 1) == 0) {                       \
    best = str;                                                                \
    bestType = name;                                                           \
